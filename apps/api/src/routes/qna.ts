...
export const qnaRoutes = new Hono<{ Variables: AuthVariables }>();
...
function anonymizeFor(items: DevQna[], viewerVendorId: string | null) {
  ...
  return items.map((q) => ({
    ...
    askerLabel:
      viewerVendorId === q.askedByVendorId ? "Du" : labelFor(q.askedByVendorId),
    askerVendorId: null,
  }));
}
...
qnaRoutes.get("/tenders/:tenderId/qna", (c) => {
  ...
  if (!tender) {
    return c.json(
      { error: { code: "NOT_FOUND", message: "Ausschreibung nicht gefunden" } },
      404,
    );
  }
